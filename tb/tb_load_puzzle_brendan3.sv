// ─────────────────────────────────────────────────────────────
// Testbench for LoadPuzzle — Puzzle_Brendan3
...
module tb_load_puzzle;
    ...
    // ── Colour constants ─────────────────────────────────────
    localparam logic [TC-1:0] X = 5'b00001;
    localparam logic [TC-1:0] A = 5'b00010;
    localparam logic [TC-1:0] B = 5'b00100;
    localparam logic [TC-1:0] C = 5'b01000;
    localparam logic [TC-1:0] D = 5'b10000;
    ...
    initial begin
        #10;
        $display("-----------------------------------------");
        $display(" LoadPuzzle Testbench — Brendan3");
        $display("-----------------------------------------");
        ...
        check_piece( 4, 0, C, B, X, A);
        check_piece( 5, 0, D, A, X, A);
        check_piece( 6, 0, C, A, X, B);
        check_piece( 7, 0, D, B, X, B);
        check_piece( 8, 0, C, C, D, D);
        ...
        check_piece( 4, 1, A, C, B, X);
        check_piece( 5, 1, A, D, A, X);
        check_piece( 6, 1, B, C, A, X);
        check_piece( 7, 1, B, D, B, X);
        check_piece( 8, 1, D, C, C, D);
        ...
        check_piece( 4, 2, X, A, C, B);
        check_piece( 5, 2, X, A, D, A);
        check_piece( 6, 2, X, B, C, A);
        check_piece( 7, 2, X, B, D, B);
        check_piece( 8, 2, D, D, C, C);
        ...
        check_piece( 4, 3, B, X, A, C);
        check_piece( 5, 3, A, X, A, D);
        check_piece( 6, 3, A, X, B, C);
        check_piece( 7, 3, B, X, B, D);
        check_piece( 8, 3, C, D, D, C);
