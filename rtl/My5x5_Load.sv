...
// My5x5_Load
//
//! Loads the initial game state for the My5x5 puzzle (5x5).
//!
//! Instantiates My5x5_Elements and CreateInitialGame.
//! No inputs, no parameters — all sizes are hardcoded.
//! Pure combinational — all outputs are constant wires.
//!
//! N=5, borderColours=5, innerColours=17, CC=23, V=25
// ─────────────────────────────────────────────────────────────
module My5x5_Load (
    // ── Tile elements ─────────────────────────────────────────
    output logic [24:0][22:0]  out_elements_top,         //! top colour for each tile.
    output logic [24:0][22:0]  out_elements_right,       //! right colour for each tile.
    output logic [24:0][22:0]  out_elements_bottom,      //! bottom colour for each tile.
    output logic [24:0][22:0]  out_elements_left,        //! left colour for each tile.

    // ── Initial colours ───────────────────────────────────────
    output logic [24:0][22:0]  out_colours_top,          //! initial top colour mask for each variable.
    output logic [24:0][22:0]  out_colours_right,        //! initial right colour mask for each variable.
    output logic [24:0][22:0]  out_colours_bottom,       //! initial bottom colour mask for each variable.
    output logic [24:0][22:0]  out_colours_left,         //! initial left colour mask for each variable.
    ...
    localparam int BORDER_COLOUR_COUNT = 5;    //! int. number of border colours.
    localparam int INNER_COLOUR_COUNT  = 17;   //! int. number of inner colours.
    localparam int CC                  = 23;   //! int. total colour count (1 + BORDER_COLOUR_COUNT + INNER_COLOUR_COUNT).
