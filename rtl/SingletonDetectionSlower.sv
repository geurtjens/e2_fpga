...
// ─────────────────────────────────────────────────────────────
module SingletonDetectionSlower #(
    parameter int N         = 4,
    ...
    // Returns the number of set bits in a VARIABLES-wide vector.
    ...
    always_comb begin
        ...
        for (int v = 0; v < VARIABLES; v++) begin
            ...
            // Only process variables that are still unassigned
            if (in_variablesIncludedMask[v]) begin

                automatic logic [VARIABLES-1:0] combined;
                automatic int                   total_count;
                ...
                total_count = popcount(combined);

                // Exactly one piece remaining — forced assignment
