...
//
//! Loads the initial game state for the My3x3_Unsolvable puzzle (3x3).
//!
//! Instantiates My3x3_Unsolvable_Elements and CreateInitialGame.
//! No inputs, no parameters — all sizes are hardcoded.
//! Pure combinational — all outputs are constant wires.
//!
//! N=3, borderColours=2, innerColours=4, CC=7, V=9
// ─────────────────────────────────────────────────────────────
module My3x3_Unsolvable_Load (
    // ── Tile elements ─────────────────────────────────────────
    output logic [8:0][6:0]  out_elements_top,         //! top colour for each tile.
    output logic [8:0][6:0]  out_elements_right,       //! right colour for each tile.
    output logic [8:0][6:0]  out_elements_bottom,      //! bottom colour for each tile.
    output logic [8:0][6:0]  out_elements_left,        //! left colour for each tile.

    // ── Initial colours ───────────────────────────────────────
    output logic [8:0][6:0]  out_colours_top,          //! initial top colour mask for each variable.
    output logic [8:0][6:0]  out_colours_right,        //! initial right colour mask for each variable.
    output logic [8:0][6:0]  out_colours_bottom,       //! initial bottom colour mask for each variable.
    output logic [8:0][6:0]  out_colours_left,         //! initial left colour mask for each variable.
    ...
    localparam int CC                  = 7;   //! int. total colour count (1 + BORDER_COLOUR_COUNT + INNER_COLOUR_COUNT).

    // ── My3x3_Unsolvable_Elements ─────────────────────────────────────────
