...
// Brendan8_Top
//
//! Top-level module for the Brendan8 puzzle (8x8).
//!
//! Wires together Brendan8_Load and Brendan8_Solve.
...
module Brendan8_Top (
    ...
    // ── Wires from Brendan8_Load ──────────────────────────────
    logic [63:0][11:0]  w_elements_top;         //! top colour for each tile.
    logic [63:0][11:0]  w_elements_right;       //! right colour for each tile.
    logic [63:0][11:0]  w_elements_bottom;      //! bottom colour for each tile.
    logic [63:0][11:0]  w_elements_left;        //! left colour for each tile.

    logic [63:0][11:0]  w_colours_top;          //! initial top colour mask for each variable.
    logic [63:0][11:0]  w_colours_right;        //! initial right colour mask for each variable.
    logic [63:0][11:0]  w_colours_bottom;       //! initial bottom colour mask for each variable.
    logic [63:0][11:0]  w_colours_left;         //! initial left colour mask for each variable.

    logic [63:0][63:0]   w_domain_r0;            //! initial domain bitmask rotation 0 for each variable.
    logic [63:0][63:0]   w_domain_r1;            //! initial domain bitmask rotation 1 for each variable.
    logic [63:0][63:0]   w_domain_r2;            //! initial domain bitmask rotation 2 for each variable.
    logic [63:0][63:0]   w_domain_r3;            //! initial domain bitmask rotation 3 for each variable.

    logic [63:0]               w_unassigned_variables; //! 1=variable position available.
    logic [63:0]               w_unassigned_tiles;     //! 1=tile available.

    // ── Brendan8_Load ─────────────────────────────────────────────
    //! Loads the initial puzzle state — tile elements, colour masks
    //! and domain bitmasks. Pure combinational, no clock required.
    Brendan8_Load loader (
