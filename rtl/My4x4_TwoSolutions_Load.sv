...
//
//! Loads the initial game state for the My4x4_TwoSolutions puzzle (4x4).
//!
//! Instantiates My4x4_TwoSolutions_Elements and CreateInitialGame.
//! No inputs, no parameters — all sizes are hardcoded.
//! Pure combinational — all outputs are constant wires.
//!
//! N=4, borderColours=2, innerColours=8, CC=11, V=16
// ─────────────────────────────────────────────────────────────
module My4x4_TwoSolutions_Load (
    // ── Tile elements ─────────────────────────────────────────
    output logic [15:0][10:0]  out_elements_top,         //! top colour for each tile.
    output logic [15:0][10:0]  out_elements_right,       //! right colour for each tile.
    output logic [15:0][10:0]  out_elements_bottom,      //! bottom colour for each tile.
    output logic [15:0][10:0]  out_elements_left,        //! left colour for each tile.

    // ── Initial colours ───────────────────────────────────────
    output logic [15:0][10:0]  out_colours_top,          //! initial top colour mask for each variable.
    output logic [15:0][10:0]  out_colours_right,        //! initial right colour mask for each variable.
    output logic [15:0][10:0]  out_colours_bottom,       //! initial bottom colour mask for each variable.
    output logic [15:0][10:0]  out_colours_left,         //! initial left colour mask for each variable.
    ...
    // ── My4x4_TwoSolutions_Elements ─────────────────────────────────────────
    //! Provides the colour masks for all 16 tiles in their base
