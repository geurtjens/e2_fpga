...
module Assignment_Complex #(
    ...
    // ── Status ────────────────────────────────────────────────
    output logic out_colours_changed //! 1 if any neighbour colour changed.
);
    ...
    always_comb begin : apply_assignment
        ...
        out_colours_changed = 1'b0;
        if ((vid % N) < N-1 &&
            out_colour_left[vid+1] != in_colour_left[vid+1])
            out_colours_changed = 1'b1;
        if ((vid / N) < N-1 &&
            out_colour_top[vid+N] != in_colour_top[vid+N])
            out_colours_changed = 1'b1;
        if ((vid % N) > 0 &&
            out_colour_right[vid-1] != in_colour_right[vid-1])
            out_colours_changed = 1'b1;
        if ((vid / N) > 0 &&
            out_colour_bottom[vid-N] != in_colour_bottom[vid-N])
            out_colours_changed = 1'b1;
    end
