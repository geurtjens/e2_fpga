// ─────────────────────────────────────────────────────────────
// My4x4_TwoSolutions_Top
//
//! Top-level module for the My4x4_TwoSolutions puzzle (4x4).
//!
//! Wires together My4x4_TwoSolutions_Load and My4x4_TwoSolutions_Solve.
//! Returns solution count as an 8-bit value.
//!
//! Target: DE10-Standard (Cyclone V)
//! Clock:  50 MHz
// ─────────────────────────────────────────────────────────────
module My4x4_TwoSolutions_Top (
    input  logic       clk,               //! 50 MHz clock.
    output logic [7:0] out_solution_count //! number of solutions found.
);

    // ── Wires from My4x4_TwoSolutions_Load ──────────────────────────────
    logic [15:0][10:0]  w_elements_top;         //! top colour for each tile.
    logic [15:0][10:0]  w_elements_right;       //! right colour for each tile.
    logic [15:0][10:0]  w_elements_bottom;      //! bottom colour for each tile.
    logic [15:0][10:0]  w_elements_left;        //! left colour for each tile.

    logic [15:0][10:0]  w_colours_top;          //! initial top colour mask for each variable.
    logic [15:0][10:0]  w_colours_right;        //! initial right colour mask for each variable.
    logic [15:0][10:0]  w_colours_bottom;       //! initial bottom colour mask for each variable.
    logic [15:0][10:0]  w_colours_left;         //! initial left colour mask for each variable.

    logic [15:0][15:0]   w_domain_r0;            //! initial domain bitmask rotation 0 for each variable.
    logic [15:0][15:0]   w_domain_r1;            //! initial domain bitmask rotation 1 for each variable.
    logic [15:0][15:0]   w_domain_r2;            //! initial domain bitmask rotation 2 for each variable.
    logic [15:0][15:0]   w_domain_r3;            //! initial domain bitmask rotation 3 for each variable.

    logic [15:0]               w_unassigned_variables; //! 1=variable position available.
    logic [15:0]               w_unassigned_tiles;     //! 1=tile available.

    // ── My4x4_TwoSolutions_Load ─────────────────────────────────────────────
    //! Loads the initial puzzle state — tile elements, colour masks
    //! and domain bitmasks. Pure combinational, no clock required.
    My4x4_TwoSolutions_Load loader (
    ...
    My4x4_TwoSolutions_Solve solver (
        .clk                     (clk),
