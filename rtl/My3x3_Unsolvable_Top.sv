// ─────────────────────────────────────────────────────────────
// My3x3_Unsolvable_Top
//
//! Top-level module for the My3x3_Unsolvable puzzle (3x3).
//!
//! Wires together My3x3_Unsolvable_Load and My3x3_Unsolvable_Solve.
//! Returns solution count as an 8-bit value.
//!
//! Target: DE10-Standard (Cyclone V)
//! Clock:  50 MHz
// ─────────────────────────────────────────────────────────────
module My3x3_Unsolvable_Top (
    input  logic       clk,               //! 50 MHz clock.
    output logic [7:0] out_solution_count //! number of solutions found.
);

    // ── Wires from My3x3_Unsolvable_Load ──────────────────────────────
    logic [8:0][6:0]  w_elements_top;         //! top colour for each tile.
    logic [8:0][6:0]  w_elements_right;       //! right colour for each tile.
    logic [8:0][6:0]  w_elements_bottom;      //! bottom colour for each tile.
    logic [8:0][6:0]  w_elements_left;        //! left colour for each tile.

    logic [8:0][6:0]  w_colours_top;          //! initial top colour mask for each variable.
    logic [8:0][6:0]  w_colours_right;        //! initial right colour mask for each variable.
    logic [8:0][6:0]  w_colours_bottom;       //! initial bottom colour mask for each variable.
    logic [8:0][6:0]  w_colours_left;         //! initial left colour mask for each variable.

    logic [8:0][8:0]   w_domain_r0;            //! initial domain bitmask rotation 0 for each variable.
    logic [8:0][8:0]   w_domain_r1;            //! initial domain bitmask rotation 1 for each variable.
    logic [8:0][8:0]   w_domain_r2;            //! initial domain bitmask rotation 2 for each variable.
    logic [8:0][8:0]   w_domain_r3;            //! initial domain bitmask rotation 3 for each variable.

    logic [8:0]               w_unassigned_variables; //! 1=variable position available.
    logic [8:0]               w_unassigned_tiles;     //! 1=tile available.

    // ── My3x3_Unsolvable_Load ─────────────────────────────────────────────
    //! Loads the initial puzzle state — tile elements, colour masks
    //! and domain bitmasks. Pure combinational, no clock required.
    My3x3_Unsolvable_Load loader (
