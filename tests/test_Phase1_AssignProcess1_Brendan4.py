...
from data_first_assignment_brendan4 import data_first_assignment_brendan4
...
@cocotb.test()
async def test_phase1_assign_process_brendan4(dut):
    await _apply(dut, data_elements_brendan4, data_initial_grid_brendan4)
    cocotb.log.info(
        f"Assigning variable={data_initial_grid_brendan4.VARIABLE_TO_ASSIGN} "
        f"tile={data_initial_grid_brendan4.TILE_TO_ASSIGN} "
        f"rotation={data_initial_grid_brendan4.ROTATION_TO_ASSIGN}"
    )
    gs = data_first_assignment_brendan4
    ...
    await data_first_assignment_brendan4.assert_dut(dut)
    cocotb.log.info("phase1 assign process brendan4 ✓")


def test_Phase1_AssignProcess1_Brendan4():
    ...
    runner.build(
        ...
        parameters={
            "N":  data_initial_grid_brendan4.N,
            "CC": data_initial_grid_brendan4.CC,
        },
        build_args=["--public-flat-rw", "-Wno-WIDTHEXPAND", "-Wno-WIDTHTRUNC"],
    )
    runner.test(
        hdl_toplevel="Phase1_AssignProcess",
        test_module="test_Phase1_AssignProcess1_Brendan4",
    )