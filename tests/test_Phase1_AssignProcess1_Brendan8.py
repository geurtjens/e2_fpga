...
@cocotb.test()
async def test_phase1_assign_process_brendan8(dut):
    await _apply(dut, data_elements_brendan8, data_initial_grid_brendan8)
    cocotb.log.info(
        f"Assigning variable={data_initial_grid_brendan8.VARIABLE_TO_ASSIGN} "
        f"tile={data_initial_grid_brendan8.TILE_TO_ASSIGN} "
        f"rotation={data_initial_grid_brendan8.ROTATION_TO_ASSIGN}"
    )
    gs = data_first_assignment_brendan8
    ...
    await data_first_assignment_brendan8.assert_dut(dut)
    cocotb.log.info("phase1 assign process brendan8 ✓")


def test_Phase1_AssignProcess1_Brendan8():
    ...
    runner.build(
        ...
        parameters={
            "N":  data_initial_grid_brendan8.N,
            "CC": data_initial_grid_brendan8.CC,
        },
        build_args=["--public-flat-rw", "-Wno-WIDTHEXPAND", "-Wno-WIDTHTRUNC"],
    )
    runner.test(
        hdl_toplevel="Phase1_AssignProcess",
        test_module="test_Phase1_AssignProcess1_Brendan8",
    )