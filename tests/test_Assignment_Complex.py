...
def reference(variable_id, tile_id, rotation,
    ...
    if x < N-1:
        ...
        if new != in_left[variable_id+1]: colours_changed = True
        out_left[variable_id+1] = new
    if y < N-1:
        new = in_top[variable_id+N] & pb
        if new != in_top[variable_id+N]: colours_changed = True
        out_top[variable_id+N] = new
    if x > 0:
        new = in_right[variable_id-1] & pl
        if new != in_right[variable_id-1]: colours_changed = True
        out_right[variable_id-1] = new
    if y > 0:
        new = in_bottom[variable_id-N] & pt
        if new != in_bottom[variable_id-N]: colours_changed = True
...
async def check(dut, variable_id, tile_id, rotation,
    ...
    (exp_top, exp_right, exp_bottom, exp_left,
     ...
     exp_uv, exp_ut, exp_cc) = reference(
    ...
    assert dut.out_colours_changed.value == int(exp_cc), \
        f"colours_changed={dut.out_colours_changed.value} exp={int(exp_cc)}"
...
async def test_other_variables_unchanged(dut):
    ...
    uv = full_mask()
    ut = full_mask()
    # assign var 0 (top-left corner — only right and below neighbours)
    await check(dut, 0, 1, 1, uv, ut, c, c, c, c, r, r, r, r)
    act_r0 = unpack_domain(dut.out_domain_r0.value.to_unsigned())
    # vars not adjacent to var 0 should be unchanged
    for v in range(2, V):
        if v != N:  # var N is below var 0 — skip
            assert act_r0[v] == ALL_DOM, \
                f"var {v} domain should be unchanged, got {act_r0[v]:#x}"
    cocotb.log.info("other variables unchanged ✓")

@cocotb.test()
async def test_colours_changed_detected(dut):
    """colours_changed=1 when a neighbour colour is restricted."""
    c  = full_colours()
    r  = full_domains()
    uv = full_mask()
    ut = full_mask()
    # assign inner var — neighbours will be restricted
    await check(dut, 5, 7, 0, uv, ut, c, c, c, c, r, r, r, r)
    cocotb.log.info("colours_changed detected ✓")

@cocotb.test()
async def test_no_colours_changed_when_already_constrained(dut):
    """colours_changed=0 when neighbour colours already match piece colours."""
    pt, pr, pb, pl = piece_colours(0, 0)
    # set neighbour colours to exactly match what piece provides
    c_top    = [CC_MASK] * V
    c_right  = [CC_MASK] * V
    c_bottom = [CC_MASK] * V
    c_left   = [CC_MASK] * V
    # var 1 is right of var 0 — its left should match var 0's right (pr)
    c_left[1] = pr
    # var N is below var 0 — its top should match var 0's bottom (pb)
    c_top[N]  = pb
    r  = full_domains()
    uv = full_mask()
    ut = full_mask()
    await check(dut, 0, 0, 0, uv, ut,
                c_top, c_right, c_bottom, c_left,
                r, r, r, r)
    assert dut.out_colours_changed.value == 0, \
        "colours_changed should be 0 when neighbours already constrained"
    cocotb.log.info("no colours_changed when already constrained ✓")
