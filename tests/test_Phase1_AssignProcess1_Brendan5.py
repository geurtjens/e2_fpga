...
from data_first_assignment_brendan5 import data_first_assignment_brendan5
...
@cocotb.test()
async def test_phase1_assign_process_brendan5(dut):
    await _apply(dut, data_elements_brendan5, data_initial_grid_brendan5)
    cocotb.log.info(
        f"Assigning variable={data_initial_grid_brendan5.VARIABLE_TO_ASSIGN} "
        f"tile={data_initial_grid_brendan5.TILE_TO_ASSIGN} "
        f"rotation={data_initial_grid_brendan5.ROTATION_TO_ASSIGN}"
    )
    gs = data_first_assignment_brendan5
    ...
    await data_first_assignment_brendan5.assert_dut(dut)
    cocotb.log.info("phase1 assign process brendan5 ✓")


def test_Phase1_AssignProcess1_Brendan5():
    ...
    runner.build(
        ...
        parameters={
            "N":  data_initial_grid_brendan5.N,
            "CC": data_initial_grid_brendan5.CC,
        },
        build_args=["--public-flat-rw", "-Wno-WIDTHEXPAND", "-Wno-WIDTHTRUNC"],
    )
    runner.test(
        hdl_toplevel="Phase1_AssignProcess",
        test_module="test_Phase1_AssignProcess1_Brendan5",
    )