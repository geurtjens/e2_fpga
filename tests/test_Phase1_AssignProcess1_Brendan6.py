...
from data_initial_grid_brendan6     import data_initial_grid_brendan6
from data_first_assignment_brendan6 import data_first_assignment_brendan6
...
@cocotb.test()
async def test_phase1_assign_process_brendan6(dut):
    await _apply(dut, data_elements_brendan6, data_initial_grid_brendan6)
    cocotb.log.info(
        f"Assigning variable={data_initial_grid_brendan6.VARIABLE_TO_ASSIGN} "
        f"tile={data_initial_grid_brendan6.TILE_TO_ASSIGN} "
        f"rotation={data_initial_grid_brendan6.ROTATION_TO_ASSIGN}"
    )
    gs = data_first_assignment_brendan6
    ...
    await data_first_assignment_brendan6.assert_dut(dut)
    cocotb.log.info("phase1 assign process brendan6 ✓")


def test_Phase1_AssignProcess1_Brendan6():
    ...
    runner.build(
        ...
        parameters={
            "N":  data_initial_grid_brendan6.N,
            "CC": data_initial_grid_brendan6.CC,
        },
        build_args=["--public-flat-rw", "-Wno-WIDTHEXPAND", "-Wno-WIDTHTRUNC"],
    )
    runner.test(
        hdl_toplevel="Phase1_AssignProcess",
        test_module="test_Phase1_AssignProcess1_Brendan6",
    )